...
class QuickAddOptions extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final cartItems = CH.cartList(ref);
    final cartRead = ref.read(cartProvider.notifier);
    final productInCart =
        cartItems.any((item) => item.productReference?.id == productId);
    if (!productInCart) {
    ...
    return DynamicList<PackagingType>(
      ...
      onSelect: (item) async => await _handleItemSelection(
          cartRead, item, selectedPackagingType, fromPsv),
      itemBuilder: (context, item, isSelected) =>
          PackagingTypeItem(item: item, isSelected: isSelected),
    );
  }

  Future<void> _handleItemSelection(Cart cartRead, PackagingType item,
      ValueNotifier<PackagingType?> selectedPackagingType, bool formPsv) async {
    if (item == selectedPackagingType.value) {
      cartRead.removeItem(productId, updateFinalCart: formPsv);
      selectedPackagingType.value = null;
    } else {
      await cartRead.addItem(productId, item.quantity,
          clearAndUpdate: true, updateFinalCart: formPsv);
