...
class StaticColors {
  ...
  final moneyGreen = Color(0xFF118C4F);
  final shukraBlue = Color(0xFF2A69AC);
  final mystic = const Color(0xFFF7F5FC);
  final nobel = const Color(0xffa9a8b5);

  // 100 shades of purple
  // final blueBell = const Color(0xFF886FF5); // onPrimaryFixedVariant
  // final lavenderGray = const Color(0xffa08ee7); surfaceTint
  // final royalPurple = const Color(0xFF5932E8); onPrimaryFixed
  // final lavenderGrayHard = Color(0xFFA08EFF); //primaryFixedDim
  // final purple = Color(0xFFBA45CF);
  // final wisteria = const Color(0xff8874ae); //inversePrimary
  ...
  final teal = Color(0xFF009688);

  //Dashboard colors
  final rejected = Color(0xFFF24D4B);
  final waiting = Color(0xFFCE983B);
  final approved = Color(0xFF14866C);
  final closed = Color(0xFF13468A);
...
// transparent 4
  ...
  Color greyTransparent40(BuildContext c) => const Color(0x40767676);
  Color greyTransparent80(BuildContext c) => const Color(0x80767676);

// background 6
  Color whiteSwNero(BuildContext c) => cFunS.ldcs(c, sc.whiteS, sc.nero);
  Color neroSwWhite(BuildContext c) => cFunS.ldcs(c, sc.nero, sc.whiteS);
  ...
  Color readOnly(BuildContext c) => cFunS.ldcs(
      c, sc.blackS.withValues(alpha: 0.25), sc.whiteS.withValues(alpha: 0.25));
  Color borderColor(BuildContext c) => cFunS.ldcs(c, sc.nero, sc.grey150);
}
...
class ColorX {
  ...
  // Seed color (default)
  static Color _seed = Colors.deepPurpleAccent;

  // Set the seed color
  static void setSeed(Color seed) => _seed = seed;

  // Getters for seed-based variations
  Color get seedForLight => _seed.withAlpha(100);
  // Color get seedForLight => _seed;
  Color get seedForDark => _seed;

  // 🔹 New: Access ColorScheme directly from context
  ColorScheme cS(BuildContext context) => Theme.of(context).colorScheme;

  // 🔹 New: Quick theme brightness check
